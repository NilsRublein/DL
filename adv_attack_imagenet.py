...
def get_labels():
    ...
    """
    csv_path = os.getcwd() + r"\data\final_dataset.csv"
    df = pd.read_csv(csv_path, usecols=['ImageId', 'TrueLabel', 'TargetClass'])
    present_images = os.listdir(path + r"\imagenet_all_images")
    labels = {}
...
for atk in atks :
    ...
    for images, labels in data_loader:
        ...
        if not os.path.isfile(r'./data/atk_images/' + str(name[0]) + '_' + str(attack_name) + '.png'): # if file doesnt exist yet, write it like this
            save_image(make_grid(adv_images.cpu().data),r'./data/atk_images_ALL/' + str(name[0]) + '_' + str(attack_name) + '.png')
            
        else: 
            print("file already exists, creating new file.")
            counter += 1
            save_image(make_grid(adv_images.cpu().data),r'./data/atk_images_ALL/' + str(name[0]) + str(counter) + '_' + str(attack_name) + '.png')
...
for images, labels in data_loader:
    ...
    img = make_grid(images.cpu().data)
    with open(img, "a") as text_file:
        text_file.write(r'./data/atk_images/' + str(name[0]) + '_' + str(attack_name) + '.png')
...
print( len( set(dataset.labels.values() )) ) # 646 ->> we have multiple images with the same label -> the attack function overrites the previous file with the same label. add some if not none function
